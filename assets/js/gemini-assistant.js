document.addEventListener("DOMContentLoaded", () => {
    ...
    // Generate AI suggested features based on selected project type
    function generateAISuggestedFeatures() {
      const selectedProject = document.querySelector(".project-type-item.selected")
      if (!selectedProject) return
  
      const projectType = selectedProject.dataset.type
  
      // Call Gemini API to get feature suggestions
      callGeminiForFeatureSuggestions(projectType)
        .then((suggestions) => {
          displayAISuggestedFeatures(suggestions)
        })
        .catch((error) => {
    ...
    // Call Gemini API for feature suggestions
    async function callGeminiForFeatureSuggestions(projectType) {
      ...
      const projectTypeNames = {
        ...
        iot: "aplicación IoT",
        ai: "aplicación de inteligencia artificial",
        monorepo: "monorepo de Node.js",
      ...
      const prompt = `
        Eres un experto en desarrollo de software. Necesito 3 características básicas y específicas para un proyecto de tipo "${projectTypeNames[projectType] || projectType}".
        
        Responde SOLO con un array JSON con exactamente 3 características. Cada característica debe tener un nombre corto y una descripción breve.
        
        Las características deben ser similares en formato y estilo a estas:
        - Diseño Responsive
        - CMS / Panel Admin
        - Optimización SEO
        - Analytics / Reportes
        ...
        [
          {
            "name": "Nombre de la característica 1",
            "description": "Breve descripción de la característica 1"
          },
          {
            "name": "Nombre de la característica 2",
            "description": "Breve descripción de la característica 2"
          },
          {
            "name": "Nombre de la característica 3",
            "description": "Breve descripción de la característica 3"
          }
        ]
        
        Las características deben ser específicas para el tipo de proyecto, prácticas y útiles. No incluyas características muy técnicas o complejas.
        `
        ...
        return [
          {
            ...
            description: "Permite exportar datos en formatos como CSV o PDF",
          },
          {
            name: "Tema Personalizable",
            description: "Opciones para personalizar colores y apariencia",
          },
          {
            name: "Búsqueda Avanzada",
            description: "Sistema de búsqueda con filtros y opciones",
          },
        ]
      }
    }
  
    // Display AI suggested features
    function displayAISuggestedFeatures(suggestions) {
      aiSuggestedFeatures.innerHTML = `
          <div class="ai-suggested-title">
            <i class="fas fa-robot"></i> Características Sugeridas por IA
          </div>
        `
  
      suggestions.forEach((suggestion, index) => {
        const featureId = `feature-ai-${index + 1}`
        const featureItem = document.createElement("div")
        featureItem.className = "ai-feature-item"
        featureItem.innerHTML = `
            <input type="checkbox" id="${featureId}" class="feature-checkbox">
            <label for="${featureId}">
              <strong>${suggestion.name}</strong>: ${suggestion.description}
            </label>
          `
        aiSuggestedFeatures.appendChild(featureItem)
  
        // Add event listener to the checkbox
        const checkbox = featureItem.querySelector(`#${featureId}`)
    ...
    // Listen for project type selection to update AI suggestions
    document.querySelectorAll(".project-type-item").forEach((item) => {
      item.addEventListener("click", () => {
        // Wait a bit for the selection to be updated
        setTimeout(() => {
          generateAISuggestedFeatures()
        }, 300)
      })
    })
  
    // Generate initial AI suggestions for default selected project
    setTimeout(() => {
      generateAISuggestedFeatures()
    }, 1000)
    ...
    function processGeminiResponse(responseText, userPrompt) {
      try {
        ...
        conversationHistory.push({
          role: "assistant",
          content: "Lo siento, ha ocurrido un error al procesar la respuesta. Por favor, intenta de nuevo.",
        })
      }
    }
